...
const result = await walrusClient.decryptFromWalrusBlobId<User>({blobId: "OmYrSA1xi1FjgkmZOmUcK6rE8j2KzfVFgPgDF_GLZVA"}, "Subscription", {serviceObjectId: keyId.id.id, subscriptionObjectId: subscription} )
console.log(result)
//const computedKeys = walrusClient.createPrivateDataKeys("secret");
// const result = await walrusClient.encryptWithPatternPrivateDataAndStoreOnChain<User>(computedKeys, {
//     id: "xxx",
...
//const result = await walrusClient.decryptFromWalrusBlobId<User>({blobId: "RV6wkElsL9VzyPjlQdaml8sfRvvccZwb3WvVWHrQRJA"}, "Private Data", undefined, computedKeys.nonce)
...
//const allowList = await walrusClient.createAllowListKey("NewAllow");
//const key = walrusClient.encryptWithPatternAllowListAndReturnBytes<User>()
